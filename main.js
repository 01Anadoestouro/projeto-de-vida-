...
const textos = document.querySelectorAll(".aba-conteudo");botoes[i].classList.add("ativo");
textos[i].classList.add("ativo");botoes[j].classList.remove("ativo");
textos[j].classList.remove("ativo");for (let i = 0; i < botoes.length; i++) {
...
contadores[0].textContent = calculaTempo(tempoObjetivo1);function calculaTempo(tempoObjetivo) {
...
let dias = Math.floor(horas / 24);return (
  dias +
  " dias " +
  horas +
  " horas " +
  ...
  return (
    dias +
    //comecaCronometro();<div class="contador">
...
const tempoObjetivo4 = new Date("2024-02-01T00:00:00");contadores[0].textContent = calculaTempo(tempoObjetivo1);
contadores[1].textContent = calculaTempo(tempoObjetivo2);
contadores[2].textContent = calculaTempo(tempoObjetivo3);
contadores[3].textContent = calculaTempo(tempoObjetivo4);const tempos = [tempoObjetivo1, tempoObjetivo2, tempoObjetivo3, tempoObjetivo4];contadores[0].textContent = calculaTempo(tempoObjetivo1); //Antes
...
comecaCronometro();function atualizaCronometro() {
  ...
  horas %= 24;
  if (tempoFinal > 0) {
    return [dias, horas, minutos, segundos];document.getElementById("seg0").textContent = calculaTempo(tempos[0])[3];
